...
class JointStatePublisher : public rclcpp::Node
{
	...
	JointStatePublisher() : Node("ik_joint_state_publisher_cpp")
	{
		...
		subscriber_ = this->create_subscription<sensor_msgs::msg::JointState>(
			...
			std::bind(&JointStatePublisher::handle_feedback_, this, std::placeholders::_1)
		);
			
		// Timer to call the callback at 10Hz
		timer_ = this->create_wall_timer(10ms, std::bind(&JointStatePublisher::timer_callback, this));
	...
	void timer_callback()
	{
		...
		double x = 0.25 + 0.04 * sin(0.1 * t);
		left_target = create_se3(1.0, 0.0, 0.0, 0, x, 0.25, 0.1);
		right_target = create_se3(1.0, 0.0, 0.0, 0, 0.25, -0.02-x, 0.1);
