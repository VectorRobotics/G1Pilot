...
class UnitreeSdk2Bridge:
    ...
    def LowCmdHandler_Joint(self, msg: JointState):
        if self.mj_data is not None:
            if msg is not None:
                # Map ROS joint names to Mujoco motor IDs
                for i, joint_name in enumerate(msg.name):
                    try:
                        # Find the motor ID in Mujoco model
                        # motor_id = self.mj_model.actuator(joint_name).id
                        motor_id = mujoco.mj_name2id(self.mj_model, mujoco.mjtObj.mjOBJ_JOINT, joint_name)
                        # Set the motor's control input (position target)

                        self.mj_data.ctrl[motor_id] = (
                            # msg.effort[i]
                            self.arm_kp * (msg.position[i] - self.mj_data.sensordata[motor_id])
                            # + self.arm_kd * (msg.velocity[i] - self.mj_data.sensordata[motor_id + self.num_motor])
                        )
                    except KeyError:
                        # Handle cases where a joint name from ROS doesn't exist in Mujoco
                        print(f"Warning: Joint '{joint_name}' not found in Mujoco model.")
                    except Exception as e:
                        print(f"Error setting motor command for {joint_name}: {e}")

                # self.mj_data.ctrl[12:14] = 
